-- Nota: Ejecutar primero estas lineas
alter session
set
    "_oracle_script" = true;
...
Alter table
    residencia
add
    tipoResidencia varchar2(30) not null;

alter table
    residencia
add
    constraint ckc_tiporesidencia check(
        tipoResidencia in(
            'Reptiliario Grande',
...
create table Empleado(
    ...
    constraint ckc_cargoEmpleado check(
        cargoEmpleado in('Veterinario', 'Auxiliar', 'Gerente', 'Vendedor')
    )
);
...
create table Producto(
    ...
    constraint ckc_tipoProducto check(
        tipoProducto in(
            'Alimento',
            'Medicamento',
            'Juguete',
...
alter table
    mascota
add
    constraint ckc_generoMascota check(generoMascota in('Macho', 'Hembra'));

alter table
    mascota
add
    constraint ckc_tipomascota check(
        tipoMascota in('Reptil', 'Perro', 'Gato', 'Anfibio', 'Pez', 'Roedor')
    );
